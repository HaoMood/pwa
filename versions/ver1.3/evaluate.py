...
class EvaluateManager(object):
    ...
    Attributes:
        ...
        _is_crop, bool: Whether to use full image or cropped image.
    """
    def __init__(self, paths, is_crop):
        self._paths = paths
    ...
    def extractQuery(self):
        ...
        self._is_crop determines whether we use full image or cropped image
        for query.
        """
        print('Extract query descriptors from all.')
        if self._is_crop:
            os.system('cp %s* %s' % (self._paths['descriptor_crop'],
                                     self._paths['descriptor_query']))
        else:
            for f in glob.iglob(os.path.join(self._paths['groundtruth'],
                ...
                os.system('cp %s.npy %s.npy' %
                          (os.path.join(self._paths['descriptor_all'],
                                        image_name),
...
def main():
    ...
    paths = {
        ...
        'descriptor_crop': os.path.join(project_root,'data/descriptor/crop/'),
        'descriptor_query': os.path.join(project_root,'data/descriptor/query/'),
        'compute_ap': os.path.join(project_root, 'lib/'),
    }
    is_crop = True;
    evaluate_manager = EvaluateManager(paths, is_crop)
